...
void Detector::Detection(const string& im_name)
{
    ...
    float *data_buf= new float[height*width*3];	// each pixel value in cv_resized
    float *boxes = NULL;
    float *pred = NULL;
    ...
    for (int i = 1; i < class_num; i ++)		// i = 0, means background
    {
		...
		caffe::Frcnn::gpu_nms(keep, &num_out, sorted_pred_cls, num, 5, NMS_THRESH, 0);
...
int main()
{
	...
	/*
    string model_file = "test.prototxt";
    string weights_file = "pvanet_frcnn_traffic_lite_noconv5_ft_iter_120000.caffemodel";
    //int GPUID=0;
    //Caffe::SetDevice(GPUID);
    Caffe::set_mode(Caffe::GPU);
    Detector det = Detector(model_file, weights_file);
    det.Detection("test/160_20171028082856120.jpg");
	*/
return 0;
}
